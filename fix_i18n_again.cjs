...
for (const [k, v] of Object.entries(nlExtra)) {
    if (!content.includes(`"${k}": "${v}"`)) {
        nlStr += `    "${k}": "${v}",\n`;
    }
}

content = content.replace('  nl: {', deStr + '  nl: {');
content = content.replace('};', nlStr + '};');
// Need to be careful because '};' matches the end of translations, but wait, translations ends with `}}` or `}};\nexport const`. 
// Let's replace 'export const I18nContext' with nlStr + '\nexport const I18nContext' and remove the trailing comma from the last property or just insert inside `nl: {`.
