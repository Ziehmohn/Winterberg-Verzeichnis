...
export default function PricingTable({ theme, activeThemeKey, onBack, onSelect, onInquireAd, hideAction = false }: PricingTableProps) {
  ...
  const features = [
    ...
    { name: 'News & Pressemitteilungen veröffentlichen (bis zu 5 / Monat)', free: false, premium: true },
  ...
  return (
    <main className="flex-1 w-full max-w-[1000px] mx-auto px-6 py-[54px] pb-[80px]">
      ...
      <p className="text-[17px] leading-[1.65] text-[#4A544D] max-w-[60ch] mb-8">
        Der Basiseintrag ist dauerhaft kostenlos. Premium schaltet Öffnungszeiten, Galerie, ausführliches Profil, Stellenanzeigen, News-Publikation, White-Label Bewertungs-Widgets und die Top-Platzierung frei.
      </p>
          ...
          <div className="text-[14px] text-[#5F6B63] mb-[22px]">
            ...
            {billingCycle === 'yearly' ? (
              <div className="mt-1.5 font-bold text-[#0F4C2E] bg-[#0F4C2E]/5 inline-block px-2 py-0.5 rounded text-[13px]">
                Gesamtbetrag: {PRICING.premiumYearlyTotal}
              </div>
            ) : (
              ` · monatlich kündbar`
            )}
      ...
      <div className="bg-gradient-to-br from-[#0F4C2E] to-[#06301C] text-white rounded-lg p-8 md:p-10 shadow-xl flex flex-col md:flex-row gap-6 items-center justify-between">
        <div className="max-w-[56ch]">
          ...
          <div className="inline-flex flex-wrap gap-2 text-xs text-white/90">
            <span className="bg-white/15 px-3 py-1 rounded">1-2 Kat.: <strong>{PRICING.bannerTier1} / Mo.</strong></span>
            <span className="bg-white/15 px-3 py-1 rounded text-emerald-300">Ab 3 Kat.: <strong>{PRICING.bannerTier2} / Mo.</strong> ({PRICING.bannerTier2Discount})</span>
            <span className="bg-white/15 px-3 py-1 rounded text-[#F2761B]">Ab 5 Kat.: <strong>{PRICING.bannerTier3} / Mo.</strong> ({PRICING.bannerTier3Discount})</span>
          </div>
          <div className="text-[12px] text-white/60 mt-2">
            Monatlich kündbar · {PRICING.cancellationPeriod} Kündigungsfrist zum Monatsende (wie beim Premium-Account)
          </div>
        </div>
        {onInquireAd && (
          <button
