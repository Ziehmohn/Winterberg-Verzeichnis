      ...
      <AnimatePresence>
        ...
        {showClaimScreen && (
          <ClaimBusinessModal
            business={business}
            currentUser={user}
            isOpen={showClaimScreen}
            onClose={() => setShowClaimScreen(false)}
            onStartPremiumCheckout={handleClaim}
        ...
        <div className="bg-white border border-[#EDE8E0] rounded-lg p-5 sm:p-7 shadow-[0_10px_30px_rgba(27,33,29,0.06)]">
            ...
            <>
              ...
              <div className="grid gap-3 mb-[30px]">
                {business.events.map(ev => {
                  const title = (lang === 'nl' && ev.title_nl) ? ev.title_nl : ev.title;
                  const description = (lang === 'nl' && ev.description_nl) ? ev.description_nl : ev.description;
                  const date = new Date(ev.startDate).toLocaleDateString(lang === 'nl' ? 'nl-NL' : 'de-DE', {
                    weekday: 'short', day: '2-digit', month: 'long'
                  });
                  return (
                    <div key={ev.id} className="border border-[#EDE8E0] bg-[#FAF8F5] rounded-xl p-4 shadow-sm flex gap-4 items-start">
                      {ev.imageUrl && (
                        <div className="w-20 h-20 sm:w-28 sm:h-28 shrink-0 rounded-lg overflow-hidden bg-gray-100 hidden sm:block">
                          <img src={ev.imageUrl} alt={title} className="w-full h-full object-cover" />
                        </div>
                      )}
                      <div className="flex-1 min-w-0">
                        <div className="flex flex-wrap items-center gap-2 mb-1.5">
                          <span className="text-[12px] text-[#0F4C2E] font-bold bg-emerald-50 px-2.5 py-0.5 rounded flex items-center gap-1 border border-emerald-100">
                            <Calendar className="w-3.5 h-3.5" />
                            {date}
                          </span>
                          <span className="bg-purple-100 text-purple-800 rounded-full px-2.5 py-0.5 text-[11px] font-semibold">
                            {ev.category}
                          </span>
                        </div>
                        <h3 className="font-display text-[17px] font-bold text-[#1B211D] mb-1 leading-snug">{title}</h3>
                        
                        {(ev.startTime || ev.endTime || ev.location) && (
                          <div className="flex flex-wrap items-center gap-3 text-xs text-[#5F6B63] mb-2 font-medium">
                            {(ev.startTime || ev.endTime) && (
                              <span className="flex items-center gap-1">
                                <Clock className="w-3 h-3 text-[#8A958E]" />
                        ...
                        {description && (
                          <p className="mt-2 text-[13.5px] text-[#4A544D] leading-[1.6] line-clamp-3">
                            {description}
                          </p>
                        )}
                        
                        {ev.externalUrl && (
                          <div className="mt-3 pt-3 border-t border-[#EDE8E0] flex justify-end">
                            <a
                              href={ev.externalUrl}
                              target="_blank"
                              rel="noopener noreferrer"
                              className="inline-flex items-center gap-1.5 px-3.5 py-1.5 rounded-lg bg-[#7E22CE] hover:bg-[#6B21A8] text-white text-xs font-semibold shadow-sm transition-all"
                            >
                              <span>{lang === 'nl' ? 'Meer info & tickets' : 'Mehr Infos & Tickets'}</span>
          ...
          {Array.isArray(business.jobs) && business.jobs.length > 0 && (
            <>
              ...
              <div className="grid gap-3 mb-[30px]">
                {business.jobs.map(j => (
                  <div key={j.id} className="border border-[#EDE8E0] bg-white rounded-xl p-4 shadow-sm">
                    <div className="flex justify-between gap-3 items-baseline flex-wrap">
                      <div>
                        <div className="font-display text-[17px] font-semibold text-[#1B211D]">{j.title}</div>
                        {j.location && <div className="text-xs text-[#8A928B] mt-0.5">{j.location}</div>}
                      </div>
                      <div className="flex items-center gap-2">
                        ...
                        <span className="bg-[#FFF1E4] text-[#D65F0C] rounded-full px-2.5 py-0.5 text-[12px] font-semibold">{j.type}</span>
                      </div>
                    </div>
                    <p className="mt-2 text-[14px] text-[#4A544D] leading-[1.55]">{j.description}</p>
                    {j.externalUrl && (
                      <div className="mt-3 pt-2.5 border-t border-gray-100 flex justify-end">
                        ...
                        >
                          <span>{lang === 'nl' ? 'Direct solliciteren' : 'Jetzt bewerben'}</span>
                          <ExternalLink className="w-3.5 h-3.5" />
                        </a>
                      </div>
                    )}
          ...
          <div className="grid gap-3 mb-5">
            {Array.isArray(business.reviews) && business.reviews.filter(r => !r.status || r.status === 'approved').length > 0 ? (
              business.reviews.filter(r => !r.status || r.status === 'approved').map(r => {
                ...
                return (
                  <div key={r.id} className="bg-[#FAF8F5] border border-[#EDE8E0] rounded-xl p-4 shadow-sm">
                    <div className="flex justify-between gap-2.5 items-center mb-1">
                      <div className="flex items-center gap-2.5">
                        ...
                        <div>
                          <div className="flex items-center gap-2 flex-wrap">
                            <span className="font-semibold text-[15px] text-[#1B211D]">{author}</span>
                            {r.source === 'google' && (
                            ...
                            {r.source && r.source !== 'google' && r.source !== 'provenexpert' && r.source !== 'internal' && (
                              <span className="inline-flex items-center gap-1 text-[11px] font-medium px-2 py-0.5 rounded-full bg-gray-100 text-gray-700 border border-gray-200">
                                {r.source}
                              </span>
                            )}
                          </div>
