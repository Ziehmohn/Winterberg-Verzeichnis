...
export function getSeoContent(category: string, location: string, businessCount: number, lang: 'de' | 'nl' = 'de'): SeoContent {
  const isNl = lang === 'nl';
  const locText = location !== 'Alle' ? location : (isNl ? 'Winterberg en omgeving' : 'Winterberg und Umgebung');
  ...
  if (isNl) {
    // Dutch SEO Content
    if (isAll) {
      introTitle = `Alle bedrijven in ${locText}`;
      introText = `Vind hier alle geregistreerde bedrijven, dienstverleners en winkels in ${locText}. In het Winterberg-overzicht staan momenteel ${businessCount} actieve ondernemingen. Steun de lokale economie en vind precies wat u zoekt.`;
      
      faqs.push({
        question: `Hoeveel bedrijven zijn er in ${locText}?`,
        answer: `In de Winterberg bedrijvengids staan momenteel ${businessCount} geregistreerde bedrijven, vakmensen en dienstverleners uit ${locText}.`
      });
      faqs.push({
        question: `Welke branches zijn vertegenwoordigd in Winterberg?`,
        answer: `Winterberg biedt een grote verscheidenheid aan sectoren: van eersteklas horeca en hotels tot lokale ambachtslieden, detailhandel en uitgebreide zakelijke dienstverlening.`
      });
    } else {
      switch (category) {
        case 'Gastronomie':
        case 'Horeca':
          introTitle = `Horeca & Restaurants in ${locText}`;
          introText = `Ontdek de culinaire diversiteit in ${locText}. Of u nu zoekt naar een gezellig café, een traditioneel Duits gasthuis of een sfeervol restaurant – hier vindt u de beste adressen. Momenteel staan er ${businessCount} horecabedrijven in de gids.`;
          faqs.push({
            question: `Hoeveel horecagelegenheden zijn er in ${locText}?`,
            answer: `In de gids staan momenteel ${businessCount} restaurants, cafés en bars in ${locText}.`
          });
          break;

        case 'Handwerk':
        case 'Ambacht':
          introTitle = `Vakmensen & Ambachten in ${locText}`;
          introText = `Op zoek naar een betrouwbare vakman in ${locText}? Of het nu gaat om een dakdekker, timmerman, elektricien of autogarage – hier vindt u alle lokale vakbedrijven op een rij (${businessCount} bedrijven geregistreerd).`;
          faqs.push({
            question: `Hoe vind ik een goede vakman in ${locText}?`,
            answer: `In de Winterberg bedrijvengids staan momenteel ${businessCount} ambachtelijke bedrijven uit ${locText}. U kunt de profielen direct bekijken en contact opnemen.`
          });
          faqs.push({
            question: `Welke vakmensen zijn actief in ${locText}?`,
            answer: `Van autogarages en timmerlieden tot dakdekkers en elektriciens vindt u in ${locText} specialisten voor vrijwel elk project.`
          });
          break;

        case 'Hotels und Unterkünfte':
        case 'Hotels en accommodaties':
          introTitle = `Hotels & Accommodaties in ${locText}`;
          introText = `Plan uw verblijf of vakantie in ${locText}. Vind hier comfortabele hotels, gezellige vakantieappartementen en pensions in het Sauerland (${businessCount} accommodaties geregistreerd).`;
          faqs.push({
            question: `Waar kan ik het beste overnachten in ${locText}?`,
            answer: `Er is een ruime keuze: in het overzicht staan momenteel ${businessCount} verschillende accommodaties in ${locText}, van luxe hotels tot gastvrije familiepensions.`
          });
          break;

        case 'Dienstleistungen':
        case 'Dienstverlening':
          introTitle = `Dienstverlening in ${locText}`;
          introText = `Van financiën en gezondheid tot marketing en juridisch advies: vind hier ${businessCount} professionele dienstverleners in ${locText}.`;
          faqs.push({
            question: `Welke diensten worden aangeboden in ${locText}?`,
            answer: `Het aanbod van de ${businessCount} dienstverleners varieert van consultancy en gezondheidszorg tot dagelijkse diensten zoals kappers en banken.`
          });
          break;

        case 'Einzelhandel':
        case 'Detailhandel':
          introTitle = `Winkelen & Detailhandel in ${locText}`;
          introText = `Lokaal winkelen in ${locText}! Ontdek winkels, boetieks en speciaalzaken. Steun de lokale ondernemers in de regio (${businessCount} winkels vermeld).`;
          faqs.push({
            question: `Welke winkels zijn er in ${locText}?`,
            answer: `In het overzicht staan momenteel ${businessCount} winkels in ${locText} met een breed assortiment voor dagelijkse behoeften, sport, mode en meer.`
          });
          break;

        default:
          introTitle = `${category} in ${locText}`;
          introText = `Vind hier de beste aanbieders en bedrijven voor ${category} in ${locText}. Momenteel staan er ${businessCount} vermeldingen in de gids met openingstijden, adressen en contactmogelijkheden.`;
          faqs.push({
            question: `Hoeveel bedrijven voor ${category} zijn er in ${locText}?`,
            answer: `In de Winterberg gids staan momenteel ${businessCount} bedrijven in de categorie ${category} in ${locText}.`
          });
      }
    }
  } else {
